...
public class EntityMissile extends EntityArrow implements IProjectile
{
	...
	public EntityMissile(World world, EntityLivingBase shooter, EntityLivingBase p_i1755_3_, float p_i1755_4_, float p_i1755_5_)
	{
		super(world);
		this.renderDistanceWeight = 10.0D;
	...
	public EntityMissile(World world, EntityLivingBase shooter, float strength, boolean allowMissileExplosions, boolean stealth)
	{
		super(world);
		this.renderDistanceWeight = 10.0D;
		this.shootingEntity = shooter;
		...
		{
			dZ = Math.cos((double)this.rotationYaw * Math.PI / 180.0D) * 1.2D;
			dX = -Math.sin((double)this.rotationYaw * Math.PI / 180.0D) * 1.2D;
		}
	...
	public void onUpdate()
	{
		...
		if (block.getMaterial() != Material.air)
		{
			...
			AxisAlignedBB axisalignedbb = block.getSelectedBoundingBox(this.worldObj, new BlockPos(this.blockX, this.blockY, this.blockZ));
		...
		if (this.inGround)
		{
			...
			this.motionY = 0;
			this.motionZ = 0;
			worldObj.createExplosion(this.shootingEntity, blockX, blockY, blockZ, 4, missileExplosions);
			this.setDead();
		}
		else
		{
			...
			for (int i = 0; i < 40; ++i)
			{
				...
				worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, posX + (rand.nextFloat() / 4), posY + (rand.nextFloat() / 4), posZ + (rand.nextFloat() / 4), 0, 0, 0);
//				worldObj.spawnParticle("flame", posX + (rand.nextFloat() / 4), posY + (rand.nextFloat() / 4), posZ + (rand.nextFloat() / 4), 0, 0, 0);
			...
			for (i = 0; i < list.size(); ++i)
			{
				...
				{
					gravity = 0.2F;
					AxisAlignedBB axisalignedbb1 = entity1.getBoundingBox().expand((double) gravity, (double) gravity, (double) gravity);
			...
			if (movingobjectposition != null)
			{
				if (entityHit != null && entityHit != shootingEntity)
				{
					...
					{
						worldObj.createExplosion(shootingEntity, entityHit.posX, entityHit.posY, entityHit.posZ, 4, missileExplosions);
						this.setDead();
					}
					else
					{
				...
				else
				{
					...
					Block hitBlock = worldObj.getBlockState(new BlockPos(blockX, blockY, blockZ)).getBlock();
					if (hitBlock.getMaterial() != Material.air)
					{
						hitBlock.onEntityCollidedWithBlock(this.worldObj, new BlockPos(this.blockX, this.blockY, this.blockZ), this);
					}
				}
			}
	...
	public void readEntityFromNBT(NBTTagCompound nbt)
	{
		...
		this.stealth = nbt.getBoolean("stealth");
