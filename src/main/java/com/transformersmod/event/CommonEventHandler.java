...
import com.transformersmod.packet.PacketBroadcastState;
import com.transformersmod.packet.PacketTransformersAction;
...
public class CommonEventHandler
{
						...
						if (shotsLeft > 0)
						{
							if (shootCooldown <= 0)
							{
								if (transformer.canShoot(player) && TFDataManager.isInVehicleMode(player))
								{
									...
									if (hasAmmo)
									{
										TransformersMod.networkWrapper.sendToServer(new PacketTransformersAction(player, action));
		...
		if (player != null)
		{
			if (!player.worldObj.isRemote)
			{
				if (event.target instanceof EntityPlayer)
				{
					...
					TransformersMod.networkWrapper.sendToDimension(new PacketBroadcastState(beingTracked), beingTracked.dimension);
					TransformersMod.networkWrapper.sendToDimension(new PacketBroadcastState(player), beingTracked.dimension);
				}
			}
		}
	}
	...
	public void onSpawn(EntityJoinWorldEvent event)
	{
		...
		if (entity instanceof EntityPlayer)
		{
			...
			{
				TransformersMod.networkWrapper.sendToDimension(new PacketBroadcastState(player), player.dimension);
				...
				{
					/*if (TransformersMod.currentVersion.isNewerUpdate(TransformersMod.newestVersion))
					{
						player.addChatMessage(new ChatComponentText(EnumChatFormatting.BLUE + "Version " + TransformersMod.newestVersion + " is now available!"));
						player.addChatMessage(new ChatComponentText(""));
						player.addChatMessage(new ChatComponentText(EnumChatFormatting.BLUE + "" + EnumChatFormatting.BOLD + "What's New: "));

						String[] updates = TransformersMod.newestVersion.changelog.split(Pattern.quote("(newline)"));
